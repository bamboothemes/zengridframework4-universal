...
if (!(class_exists('ZenToolsResizeImageHelper'))) {
	...
	{
		public static function getResizedImage($image, $newWidth, $newHeight, $option='crop', $folder, $quality='90')
		{
		
			$image = preg_replace('/^\/media\//', 'media/', $image);
			...
			$imageCacheUrl = 'media/presto-gallery/cache/images/';
			$imageCacheRoot = JPath::clean(JPATH_SITE . '/'. $folder);
			...
			if (JFile::exists($imageCacheRoot . $newImage . $extension))
			{
			...
			if ($img !== false) {
				...
				if (($width < $newWidth) && ($height < $newHeight)){
					$originalImage = str_replace(ROOT_PATH . '/', '', $image);
